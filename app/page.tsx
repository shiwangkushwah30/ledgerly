...
import { InteractiveMoneyCalendar } from '@/components/money-calendar'
import {
  ArrowDownRight,
  ArrowUpRight,
  Bell,
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const storedTransactions = window.localStorage.getItem('pennywise-transactions')
    const storedBalance = window.localStorage.getItem('pennywise-balance')
  ...
  useEffect(() => {
    if (transactions.length > 0) window.localStorage.setItem('pennywise-transactions', JSON.stringify(transactions))
  }, [transactions])

  function updateBalance(value: string) {
    setBalance(value)
    window.localStorage.setItem('pennywise-balance', value)
  }
    ...
    <main className="min-h-screen bg-[#f7f8f6] text-[#18221e]">
      ...
      <div className="lg:pl-[242px]">
        ...
        <div className="mx-auto max-w-[1320px] px-5 py-8 sm:px-8 lg:px-10">
          <section id="overview" className="mb-9 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-2 text-xs font-semibold text-[#93a098]">{currentDate ? `${currentDate.toLocaleDateString('en-IN', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })} · ${currentDate.toLocaleTimeString('en-IN', { hour: 'numeric', minute: '2-digit' })}` : 'Loading local time…'}</p><h1 className="text-[30px] font-bold tracking-[-0.04em] text-[#1c2922] sm:text-[34px]">Good morning, Jordan <span className="text-[#0e9f6e]">.</span></h1><p className="mt-2 text-sm text-[#829088]">Here&apos;s your financial snapshot for June.</p></div><div className="flex flex-wrap gap-3"><a href="/add-expense" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#0e9f6e] px-5 text-sm font-bold text-white shadow-[0_5px_12px_rgba(14,159,110,.18)] transition hover:bg-[#087f58]"><Plus size={18} /> Add expense</a><button onClick={() => setShowForm(true)} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-[#dfe7e1] bg-white px-4 text-sm font-bold text-[#526159] hover:bg-[#f3f6f3]">Quick add</button></div></section>

          <section className="grid gap-4 md:grid-cols-3" aria-label="Financial summary"><SummaryCard label="Total balance" amount={balance} editable onChange={updateBalance} note="Edit your current balance" icon={Wallet} tone="green" /><SummaryCard label="Spent this month" amount={transactions.filter((item) => item.type === 'expense').reduce((sum, item) => sum + item.amount, 0).toFixed(2)} note="From your saved expenses" icon={ArrowDownRight} tone="peach" /><SummaryCard label="Money in" amount={transactions.filter((item) => item.type === 'income').reduce((sum, item) => sum + item.amount, 0).toFixed(2)} note="From your saved income" icon={ArrowUpRight} tone="blue" /></section>

          <InteractiveMoneyCalendar transactions={transactions} />
          ...
          <section className="mt-7 grid gap-5 xl:grid-cols-[1.6fr_1fr]">
            ...
            <div id="budgets" className="rounded-2xl border border-[#e5eae6] bg-white p-6 shadow-[0_2px_8px_rgba(25,55,40,.025)]"><div className="mb-7 flex items-start justify-between"><div><h2 className="text-[15px] font-bold">Budget progress</h2><p className="mt-1 text-xs text-[#93a098]">{currentDate ? `${currentDate.toLocaleString('en-IN', { month: 'long' })} spending limits` : 'Loading spending limits'}</p></div><a href="#budgets" className="text-xs font-bold text-[#0e9f6e]">View all</a></div><div className="flex flex-col gap-6"><Budget label="Housing" spent="₹1,200" total="₹1,500" percent={80} color="bg-[#61b39a]" /><Budget label="Food & dining" spent="₹426" total="₹600" percent={71} color="bg-[#f1ac72]" /><Budget label="Transport" spent="₹118" total="₹300" percent={39} color="bg-[#9b91df]" /><Budget label="Entertainment" spent="₹98" total="₹150" percent={65} color="bg-[#e98282]" /></div></div>
          </section>
...
function SummaryCard({ label, amount, note, icon: Icon, tone, editable, onChange }: { label: string; amount: string; note: string; icon: typeof Wallet; tone: string; editable?: boolean; onChange?: (value: string) => void }) { return <div className="rounded-2xl border border-[#e5eae6] bg-white p-5 shadow-[0_2px_8px_rgba(25,55,40,.025)]"><div className="flex items-center justify-between"><p className="text-xs font-semibold text-[#89958e]">{label}</p><span className={`grid size-8 place-items-center rounded-lg ₹{tone === 'green' ? 'bg-[#e6f7ef] text-[#0e9f6e]' : tone === 'peach' ? 'bg-[#fff0e7] text-[#df8a58]' : 'bg-[#eaf2ff] text-[#6f91d7]'}`}><Icon size={16} /></span></div>{editable ? <label className="mt-3 flex items-center gap-1 text-[25px] font-bold tracking-[-0.04em] text-[#18221e]"><span>₹</span><input aria-label="Total balance" type="number" step="0.01" value={amount} onChange={(event) => onChange?.(event.target.value)} className="min-w-0 w-full bg-transparent outline-none" /></label> : <p className="mt-4 text-[25px] font-bold tracking-[-0.04em]">₹{amount}</p>}<p className="mt-2 text-[11px] font-semibold text-[#35a279]">{note}</p></div> }
...
const dailySpending = Array.from({ length: 31 }, () => 0)

function MoneyCalendar({ transactions }: { transactions: Transaction[] }) {
